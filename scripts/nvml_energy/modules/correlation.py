...
def create_correlation_data(energy_files, region_file):
    energy_relative_file = None
    for f in energy_files:
        if 'relative' in f:
            energy_relative_file = f
            break
    
    if not energy_relative_file:
        print("ERROR: No relative energy file found for correlation")
        return pd.DataFrame()
    
    energy_df = pd.read_csv(energy_relative_file)
    ...
    for _, row in energy_df.iterrows():
        ...
        energy = row['energy_relative_joules']
...
def create_time_series_data(correlation_df):
    ...
    for _, row in correlation_df.iterrows():
        ...
        if len(time_idx) == 1:
            result_df.at[time_idx[0], region] = energy
    return result_df

def generate_sql_schema(series_df, output_dir, table_name='nvml_energy_series'):
    if series_df.empty:
        return
    ...
    with open(sql_path, 'w') as f:
        ...
        f.write("\n);\n\n")
        f.write(f"\\COPY {table_name} FROM '/csv_data/nvml_energy/{table_name}.csv' WITH (FORMAT csv, HEADER true);\n\n")
